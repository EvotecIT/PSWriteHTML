Function New-HTML {
    ...
    if ($Hosted.IsPresent) {
        ...
        if ($FilePath -eq '') {
            Write-Warning -Message "New-HTML - Choosing hosted option requires FilePath to be present. Terminating"
            return
        }
        $Script:HTMLSchema['Hosted']['Folder'] = Split-Path -Path $FilePath
    }
